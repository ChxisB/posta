...
| ---- | ---- | ----------- | ------- |
| `POSTA_WEB_HOSTNAME` | String | The hostname that the Posta web interface runs on | posta.example.com |
| `POSTA_WEB_PROTOCOL` | String | The HTTP protocol to use for the Posta web interface | https |
| `POSTA_SMTP_HOSTNAME` | String | The hostname that the Posta SMTP server runs on | posta.example.com |
| `POSTA_USE_IP_POOLS` | Boolean | Should IP pools be enabled for this installation? | false |
| `POSTA_DEFAULT_MAXIMUM_DELIVERY_ATTEMPTS` | Integer | The maximum number of delivery attempts | 18 |
| `POSTA_DEFAULT_MAXIMUM_HOLD_EXPIRY_DAYS` | Integer | The number of days to hold a message before they will be expired | 7 |
| `POSTA_DEFAULT_SUPPRESSION_LIST_AUTOMATIC_REMOVAL_DAYS` | Integer | The number of days an address will remain in a suppression list before being removed | 30 |
| `POSTA_DEFAULT_SPAM_THRESHOLD` | Integer | The default threshold at which a message should be treated as spam | 5 |
| `POSTA_DEFAULT_SPAM_FAILURE_THRESHOLD` | Integer | The default threshold at which a message should be treated as spam failure | 20 |
| `POSTA_USE_LOCAL_NS_FOR_DOMAIN_VERIFICATION` | Boolean | Domain verification and checking usually checks with a domain's nameserver. Enable this to check with the server's local nameservers. | false |
| `POSTA_USE_RESENT_SENDER_HEADER` | Boolean | Append a Resend-Sender header to all outgoing e-mails | true |
| `POSTA_SIGNING_KEY_PATH` | String | Path to the private key used for signing | $config-file-root/signing.key |
| `POSTA_SMTP_RELAYS` | Array of strings | An array of SMTP relays in the format of smtp://host:port | [] |
| `POSTA_TRUSTED_PROXIES` | Array of strings | An array of IP addresses to trust for proxying requests to Posta (in addition to localhost addresses) | [] |
| `POSTA_ALLOWED_REQUEST_DESTINATIONS` | Array of strings | Hostnames or IP/CIDR ranges that outbound webhook and HTTP endpoint requests are permitted to reach even when they resolve to a private, loopback, link-local or otherwise reserved address. All other such destinations are blocked to prevent SSRF. | [] |
| `POSTA_QUEUED_MESSAGE_LOCK_STALE_DAYS` | Integer | The number of days after which to consider a lock as stale. Messages with stale locks will be removed and not retried. | 1 |
| `POSTA_BATCH_QUEUED_MESSAGES` | Boolean | When enabled queued messages will be de-queued in batches based on their destination | true |
...
| `MAIN_DB_USERNAME` | String | The MariaDB username | posta |
| `MAIN_DB_PASSWORD` | String | The MariaDB password |  |
| `MAIN_DB_DATABASE` | String | The MariaDB database name | posta |
...
| `MESSAGE_DB_USERNAME` | String | The MariaDB username | posta |
...
| `MESSAGE_DB_DATABASE_NAME_PREFIX` | String | The MariaDB prefix to add to database names | posta |
| `LOGGING_ENABLED` | Boolean | Enable the Posta logger to log to STDOUT | true |
...
| `GELF_FACILITY` | String | The facility name to add to all log entries sent to GELF | posta |
...
| `DNS_MX_RECORDS` | Array of strings | The names of the default MX records | ["mx1.posta.example.com", "mx2.posta.example.com"] |
| `DNS_SPF_INCLUDE` | String | The location of the SPF record | spf.posta.example.com |
| `DNS_RETURN_PATH_DOMAIN` | String | The return path hostname | rp.posta.example.com |
| `DNS_ROUTE_DOMAIN` | String | The domain to use for hosting route-specific addresses | routes.posta.example.com |
| `DNS_TRACK_DOMAIN` | String | The CNAME which tracking domains should be pointed to | track.posta.example.com |
| `DNS_HELO_HOSTNAME` | String | The hostname to use in HELO/EHLO when connecting to external SMTP servers |  |
| `DNS_DKIM_IDENTIFIER` | String | The identifier to use for DKIM keys in DNS records | posta |
| `DNS_DOMAIN_VERIFY_PREFIX` | String | The prefix to add before TXT record verification string | posta-verification |
| `DNS_CUSTOM_RETURN_PATH_PREFIX` | String | The domain to use on external domains which points to the Posta return path domain | psrp |
...
| `SMTP_FROM_NAME` | String | The name to use as the from name outgoing emails from Posta | Posta |
| `SMTP_FROM_ADDRESS` | String | The e-mail to use as the from address outgoing emails from Posta | posta@example.com |
| `CLERK_SECRET_KEY` | String | The Clerk secret key for authentication |  |
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` | String | The Clerk publishable key for the frontend |  |
