...
export default function StepSending({ state, updateState, onNext, onBack }: Props) {
      ...
      <div className="space-y-5 max-w-lg">
        <div className="flex items-center gap-3 rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Endpoint and route created. Mail sent to your domain will be forwarded to the webhook URL.
            </p>
          </div>
        </div>
        <div className="flex gap-3 pt-2">
  ...
  return (
    <div className="space-y-5 max-w-lg">
      <p className="text-sm text-muted-foreground">
        What kind of email do you want to handle? You can change this later.
      </p>

      {/* Outgoing */}
      <button
        onClick={() => handleDirectionSelect('outgoing')}
        className="w-full text-left rounded-lg border border-border/60 p-4 hover:border-primary/30 hover:bg-primary/5 transition-all group"
      >
        <div className="flex items-start gap-4">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500/15 to-blue-500/5 text-blue-600 dark:text-blue-400 ring-1 ring-blue-500/20">
      ...
      >
        <div className="flex items-start gap-4">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500/15 to-emerald-500/5 text-emerald-600 dark:text-emerald-400 ring-1 ring-emerald-500/20">
      ...
      {/* Incoming setup form */}
      {direction === 'incoming' && (
        <form onSubmit={handleSetupIncoming} className="space-y-4 border border-border/60 rounded-lg p-4 mt-2">
